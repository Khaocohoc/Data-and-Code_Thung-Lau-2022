...
kdh %>% 
  group_by(`Loaihinh`) %>%
  tally()
kdh_Loaihinh_tally <- kdh %>% 
  group_by(`Loaihinh`) %>% 
  tally() %>% 
  filter(`Loaihinh` != 'NA') %>%  # REMOVE NAs 
  arrange(desc(n))
ggplot(kdh_Loaihinh_tally,
...
kdh %>% 
  group_by(`Mothaimat`) %>%
  tally()
kdh_Mothaimat_tally <- kdh %>% 
  group_by(`Mothaimat`) %>% 
  tally() %>% 
  filter(`Mothaimat` != 'NA') %>%  # REMOVE NAs 
  arrange(desc(n))
ggplot(kdh_Mothaimat_tally,
       aes(x = reorder(`Mothaimat`, n), y = n)) +
