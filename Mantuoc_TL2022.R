...
# Boxplot for types of stonelibra artefacts
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
mt <- read_xlsx("Manhtuoc_TL22.xlsx", sheet = "Data")
mt
view(mt)
mt %>% 
  group_by(`Tinhtrang`) %>%
  tally()
mt_Tinhtrang_tally <- mt %>% 
  group_by(`Tinhtrang`) %>% 
  tally() %>% 
  filter(`Tinhtrang` != 'NA') %>%  # REMOVE NAs 
  arrange(desc(n))
ggplot(mt_Tinhtrang_tally,
       aes(x = reorder(`Tinhtrang`, n), y = n)) +
  geom_bar(stat="identity", size = 0.6, 
           fill = "white", color = "black", 
           width = 0.8) +
  theme_bw() +
  theme(text = element_text(size=25, angle = 0, colour = 'black'),   # cach tang kich thuoc SIZE CHU
...
# Tim hieu so luong cac cong cu duoc ghe deo mot mat va hai mat_Khongdinhinh
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
...
mt %>% 
  group_by(`Nguyenlieu`) %>%
  tally()
mt_Nguyenlieu_tally <- mt %>% 
  group_by(`Nguyenlieu`) %>% 
  tally() %>% 
  filter(`Nguyenlieu` != 'NA') %>%  # REMOVE NAs 
  arrange(desc(n))
ggplot(mt_Nguyenlieu_tally,
       aes(x = reorder(`Nguyenlieu`, n), y = n)) +
...
mt <- read_xlsx("Manhtuoc_TL22.xlsx", sheet = "Data")
mt
ggplot(mt, aes(x=Nguyenlieu, y=Lop, fill=Nguyenlieu)) +
  geom_bar(stat="identity", position = "fill", size = 16) + 
  facet_grid(~ Tinhtrang) + 
...
kdh <- read_xlsx("CongcuhachdaThungLau_2022.xlsx", sheet = "kdh")
kdh
ggplot(kdh, aes(x = reorder(Loaihinh, Trongluong),
                y = Trongluong)) +
...
kdh <- read_xlsx("CongcuhachdaThungLau_2022.xlsx", sheet = "kdh")
kdh
ggplot(kdh, aes(x = reorder(Loaihinh, Dai),
                y = Dai)) +
  geom_boxplot(size = 0.8, width = 0.6,
               palette = "jco") +
  geom_point(size = 3, alpha = 1/2) +
...
kdh
attach(kdh)
ggplot(kdh, aes(x = Loaihinh, y = Trongluong)) + 
  geom_boxplot() +
  coord_flip()
