#Recalling dataset from Hang Chuong _ 2022
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
...
names(hd1)
colnames(hd1)
ggplot(hd1, aes(Toado, fill = Tinhtrang)) +
  geom_density(fill = 'darkgreen', alpha = 0, na.rm = TRUE) +
  theme_bw() +
...
hd1 %>% 
  group_by(`Loaihinh`) %>%
  tally()
hd1_Loaihinh_tally <- hd1 %>% 
  group_by(`Loaihinh`) %>% 
  tally() %>% 
  filter(`Loaihinh` != 'NA') %>%  # REMOVE NAs 
  arrange(desc(n))
ggplot(hd1_Loaihinh_tally,
...
hd1 %>% 
  group_by(`Nguyenlieu`) %>%
  tally()
hd1_Nguyenlieu_tally <- hd1 %>% 
  group_by(`Nguyenlieu`) %>% 
  tally() %>% 
  filter(`Nguyenlieu` != 'NA') %>%  # REMOVE NAs 
  arrange(desc(n))
ggplot(hd1_Nguyenlieu_tally,
...
# Phan bo nguyen lieu theo Lop-Danguyenlieu
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
...
hd1
str(hd1)
ggplot(hd1, aes(x = reorder(Nguyenlieu, Toado),
                y = Dai)) +
